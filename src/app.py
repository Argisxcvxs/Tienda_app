...
def productos_disponibles():
    if 'usuario_id' not in session or not session.get('is_admin'):
        return redirect(url_for('login'))
    productos = Producto.query.filter(Producto.stock > 0).all()  # Productos con stock disponible
...
def productos_agotados():
    if 'usuario_id' not in session or not session.get('is_admin'):
        return redirect(url_for('login'))
    productos = Producto.query.filter(Producto.stock == 0).all()  # Productos sin stock
    return render_template('productos_agotados.html', productos=productos)

# Subruta para informe de ventas (productos vendidos)
from datetime import datetime
